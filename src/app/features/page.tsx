          ...
          <div className="space-y-32">
            {features.map((feature, i) => (
              <div key={i} className={`flex flex-col md:flex-row gap-12 items-center ${i % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="flex-1 w-full aspect-video rounded-2xl bg-[#0A0A0A] border border-white/5 flex items-center justify-center overflow-hidden">
                  <div className="text-zinc-600 text-sm font-medium tracking-widest uppercase">Feature Demo</div>
                </div>
              </div>
            ))}
          </div>
